...
export function useShopDomain() {
  ...
  const fetchStatusFromEdge = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn("useShopDomain: shopify-status returnerede fejl", {
          clerkId: user?.id,
  ...
  const resolveSupabaseUserId = useCallback(async () => {
    ...
    return null;
  }, [supabase, user, getToken, supabaseUserIdHint]);
  ...
  const fetchDomain = useCallback(async () => {
    ...
    try {
      ...
      console.log("useShopDomain: fetched shop row", { clerkId: user.id, supabaseUserId, data: shopRow });
      return resolved;
    } catch (err) {
